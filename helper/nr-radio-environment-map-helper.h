 ...
 *
 * Two general types of maps for the DL can be generated according to whether
 * the BeamShape or CoverageArea is selected. Notice that the first case
 * considers the configuration of the beamforming vectors (for each transmitting
 * device) as defined by the user in the scenario script for which the map is
 * generated. In the second case, the beams are reconfigured during the map
 * generation for each rem point in order to visualize the coverage area in
 * terms of SNR and SINR and rxPower.
 *
 * Two UL REM maps can also be generated by selecting UlRem/UeCoverage.
 * In the first case a BeamShape map is generated that calculates the rxPower
 * form eact transmitting device in each Rem Point. In the second case, the map
 * visualizes the coverage area of a tx device, while there is the possibility
 * to add interference from DL gNB devices to study a worst case TDD scenario.
 * For the UL REM maps, the user has to pass to the rem-helper the NodeContainer
 * of the tx devices (UEs) and a NetDevice (gNB) form which the rem point will
 * get the configuration (antenna, txPower, etc)
 *
...
private:
   ...
   * and the SNR/SINR/rxPower values as resulted from the calculations
   */
  struct RemPoint
  {
    Vector pos {0,0,0};
