...
//  mobModel->SetPosition (Vector (dist, 0.0, 0.0));
  ...
  while (true)
    {
      int tbs = amc->CalculateTbSize (mcsTmp, 22 * config->GetBandwidthInRbs ());
      amc->CreateCqiFeedbackWbTdma (specVals, tbs, mcs);
...
//	LogComponentEnable ("MmWaveFlexTtiMacScheduler", LOG_LEVEL_DEBUG);
  ...
  Ptr<MmWavePhyMacCommon> mmWavePhyMacCommon =  CreateObject<MmWavePhyMacCommon>();
  mmWavePhyMacCommon->DoInitialize ();
