...
{
    static TypeId tid =
        TypeId("ns3::HexagonalWraparoundModel").SetParent<Object>().SetGroupName("Propagation");
    return tid;
}
...
{
    return (a - GetRelativeVirtualPosition(a, b)).GetLength();
}
...
HexagonalWraparoundModel::GetSitePosition(const Vector3D& pos) const
{
    ...
    std::transform(
        m_sitePositions.begin(),
        m_sitePositions.end(),
        virtDists.begin(),
        [&](const Vector3D& sitePos) {
            return (this->GetRelativeVirtualPosition(sitePos, pos) - sitePos).GetLength();
        });
...
Vector3D
HexagonalWraparoundModel::GetRelativeVirtualPosition(const Vector3D& absPos1,
                                                     const Vector3D& absPos2) const
{
