    ...
    static TypeId tid =
        TypeId("ns3::NrUePowerControl")
            ...
            .AddAttribute("AccumulationEnabled",
                          "If true TPC accumulation mode will be active, otherwise absolute mode "
                          "will be active",
                          BooleanValue(true),
...
NrUePowerControl::ReportTpcPusch(uint8_t tpc)
{
    ...
    if (m_accumulationEnabled)
    {
        ...
        NS_LOG_INFO("Reported TPC: " << (int)tpc << " delta accumulated: "
                                     << GetAccumulatedDelta(tpc) << " Fc: " << m_fc);
    }
    else
    {
        m_deltaPusch.push_back(GetAbsoluteDelta(tpc));
        NS_LOG_INFO("Reported TPC: " << (int)tpc << " delta absolute: " << GetAbsoluteDelta(tpc)
                                     << " Fc: " << m_fc);
    }
    ...
    if (m_technicalSpec == TS_36_213)
    {
        ...
        if (m_accumulationEnabled)
        {
            if (m_deltaPusch.size() ==
                m_k_PUSCH) // the feedback/report that should be used is from (i-m_k_PUSCH) report
            {
...
NrUePowerControl::ReportTpcPucch(uint8_t tpc)
{
    ...
    if (m_technicalSpec == TS_36_213)
    {
        ...
        if (m_deltaPucch.size() ==
            m_k_PUCCH) // the feedback/report that should be used is from (i-m_k_PUSCH) report
        {
            if ((m_curPucchTxPower <= m_Pcmin && m_deltaPucch.at(0) < 0) ||
                (m_curPucchTxPower >= m_Pcmax && m_deltaPucch.at(0) > 0))
            {
                // TPC commands for should not be accumulated because the maximum or minimum is
                // reached
            ...
            else
            {
                m_gc = m_gc +
                       m_deltaPucch.at(
                           0); // gc(i) = gc (i-1) + delta (i- KPUCCH) for TDD and FDD-TDD TS 36.213
                m_deltaPucch.erase(m_deltaPucch.begin());
            }
        }
    }
...
{
    NS_LOG_FUNCTION(this);
    NS_ABORT_MSG_IF(m_technicalSpec != TS_38_213,
                    "This function is currently being used only for TS 38.213. ");
    ...
    else
    {
        if (!m_deltaPusch.empty())
        {
            ...
            m_deltaPusch
                .pop_back(); // use the last received absolute TPC command ( 7.1.1 UE behaviour)
            m_deltaPusch.clear();
        }
    }
}
...
// TS 38.213 Table 7.1.1-1 and Table 7.2.1-1,  Mapping of TPC Command Field in DCI to accumulated
// and absolute value
...
NrUePowerControl::CalculatePuschTxPowerNr(std::size_t rbNum)
{
    ...
    NS_LOG_INFO("RBs: " << rbNum << " m_PoPusch: " << PoPusch << " Alpha: " << m_alpha
                        << " PathLoss: " << m_pathLoss << " deltaTF: " << m_deltaTF
                        << " fc: " << m_fc << " numerology:" << m_nrUePhy->GetNumerology());
     ...
     *  f of serving cell c
     *  m_pathloss = referenceSignalPower – higher layer filtered RSRP, where referenceSignalPower
     * is provided by higher layers and RSRP is defined in [7, TS 38.215] for the reference serving
     * cell and the higher layer filter configuration is defined in [12, TS 38.331] for the
     ...
     *  According to 38.213 2.1.1. If the PUSCH transmission is over more than one layer [6,
     * TS 38.214], then deltaTF is 0.
     *
     *  fc is accumulation or current absolute (calculation by using correction values received in
     * TPC commands)
     */
    ...
    NS_LOG_INFO("Calculated PUSCH power:" << txPower << " MinPower: " << m_Pcmin
                                          << " MaxPower:" << m_Pcmax);

    txPower = std::min(std::max(m_Pcmin, txPower), m_Pcmax);

    NS_LOG_INFO("PUSCH TxPower after min/max constraints: " << txPower << " for cellId/rnti: "
                                                            << m_cellId << "," << m_rnti);
...
NrUePowerControl::CalculatePucchTxPowerNr(std::size_t rbNum)
{
    ...
    NS_LOG_INFO("RBs: " << rbNum << " m_PoPucch: " << PoPucch << " Alpha: " << m_alpha
                        << " PathLoss: " << m_pathLoss << " deltaTF: " << m_deltaTF_control
                        << " gc: " << m_gc << " numerology: " << m_nrUePhy->GetNumerology());

    double txPower = PoPucch + pucchComponent + m_alpha * m_pathLoss + m_delta_F_Pucch +
                     m_deltaTF_control + m_gc;

    NS_LOG_INFO("Calculated PUCCH power: " << txPower << " MinPower: " << m_Pcmin
                                           << " MaxPower:" << m_Pcmax);

    txPower = std::min(std::max(m_Pcmin, txPower), m_Pcmax);

    NS_LOG_INFO("PUCCH TxPower after min/max constraints: " << txPower << " for cellId/rnti: "
                                                            << m_cellId << "," << m_rnti);
...
NrUePowerControl::CalculateSrsTxPowerNr(std::size_t rbNum)
{
    ...
    {
        txPower = m_P_0_SRS + component + m_alpha * m_pathLoss +
                  m_hc; // this formula also can apply for TS_36_213,
    ...
    NS_LOG_INFO("SrsTxPower after min/max constraints: " << m_curSrsTxPower << " for cellId/rnti: "
                                                         << m_cellId << "," << m_rnti);
