  ...
  if (it1 != m_connectedPair.end ())
    {
      ...
      txAntennaArray->SetBeamformingVector (bfParams->m_txW, std::make_pair (0,0), rxDevice);
      rxAntennaArray->SetBeamformingVector (bfParams->m_rxW,std::make_pair (0,0), txDevice);
    }
  else
    {
