...
MmWaveBeamforming::SetBeamformingVector (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  ...
  ueAntennaArray->SetBeamformingVector (bfParams->m_ueW, std::make_pair (0,0));
  enbAntennaArray->SetBeamformingVector (bfParams->m_enbW, std::make_pair (0,0), ueDevice);
}
  ...
  while (vit != tempPsd->ValuesEnd ())
    {
      ...
      if ((*vit) != 0.00)
        {
          double fsb = m_phyMacConfig->GetCenterFrequency () - GetSystemBandwidth () / 2 + m_phyMacConfig->GetSubcarrierSpacing () *
            m_phyMacConfig->GetNumScsPerRb () * iSubband;
                                                 ...
                                                 Ptr<const MobilityModel> b) const
{
  ...
  if (enbAntennaArray->IsOmniTx ())
    ...
    {
      complexVector_t ueW = AntennaArrayModel::GetVector (ueAntennaArray->GetCurrentBeamformingVector ());
      complexVector_t enbW = AntennaArrayModel::GetVector (enbAntennaArray->GetCurrentBeamformingVector ());
...
{
  double bw = 0.00;
  bw = m_phyMacConfig->GetSubcarrierSpacing () * m_phyMacConfig->GetNumRefScPerRb ()
    * m_phyMacConfig->GetBandwidthInRbs ();
  return bw;
}
void
