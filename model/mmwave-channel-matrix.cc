...
MmWaveChannelMatrix::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MmWaveChannelMatrix")
    ...
    .AddAttribute ("NumScPerRb",
                   "Number of sub-carriers in one resource block",
                   UintegerValue (12),
                   MakeUintegerAccessor (&MmWaveChannelMatrix::m_numScPerRb),
                   MakeUintegerChecker<uint32_t> ());
  return tid;
}
                                                   ...
                                                   Ptr<const MobilityModel> b) const
{
  ...
  if (it == m_channelMatrixMap.end ())
    {
      ...
      double bandwidth = m_numRB * m_numScPerRb;
      for (int i = 0; i < clusterNum; i++)
        {
  ...
  bfParams->m_txW = AntennaArrayModel::GetVector (txAntennaArray->GetCurrentBeamformingVector ());
  bfParams->m_rxW = AntennaArrayModel::GetVector (rxAntennaArray->GetCurrentBeamformingVector ());
...
{
  double bw = 0.00;
  bw = m_phyMacConfig->GetSubcarrierSpacing () * m_phyMacConfig->GetNumScsPerRb () * m_phyMacConfig->GetBandwidthInRbs ();
  return bw;
}
