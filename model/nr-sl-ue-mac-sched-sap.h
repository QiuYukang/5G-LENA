...
class NrSlUeMacSchedSapProvider
{
  public:
    ...
    struct NrSlSlotInfo
    {
        ...
        uint16_t slSubchannelSize{
            std::numeric_limits<uint16_t>::max()}; //!< Indicates the subchannel size in number of
                                                   //!< RBs
        uint16_t slMaxNumPerReserve{
    ...
    struct SchedUeNrSlReportBufferStatusParams
    {
         ...
         */
        SchedUeNrSlReportBufferStatusParams(uint16_t rnti,
                                            uint8_t lcId,
    ...
    virtual void SchedUeNrSlRlcBufferReq(
        const struct SchedUeNrSlReportBufferStatusParams& params) = 0;
    /**
