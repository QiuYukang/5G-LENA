 ...
 */
struct NrSlReportBufferStatusParams
{
    /**
     ...
     */
    NrSlReportBufferStatusParams(uint16_t rnti,
                                 uint8_t lcId,
    ...
    uint16_t txQueueHolDelay{std::numeric_limits<uint16_t>::max()}; //!< the Head Of Line delay of
                                                                    //!< the transmission queue
    uint32_t retxQueueSize{0}; //!< the current size of the RLC retransmission queue in bytes
    uint16_t retxQueueHolDelay{std::numeric_limits<uint16_t>::max()}; //!< the Head Of Line delay of
 ...
 */
std::ostream& operator<<(std::ostream& os, const NrSlReportBufferStatusParams& p);
