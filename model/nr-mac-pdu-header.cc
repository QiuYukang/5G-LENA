...
NS_OBJECT_ENSURE_REGISTERED (NrMacPduHeader);

NrMacPduHeader::NrMacPduHeader () : m_headerSize (0)
{
}

TypeId
NrMacPduHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrMacPduHeader")
    .SetParent<Header> ()
    .AddConstructor<NrMacPduHeader> ();
  return tid;
}

TypeId
NrMacPduHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t
NrMacPduHeader::GetSerializedSize (void) const
{
  return (m_headerSize);
}

void
NrMacPduHeader::Serialize (Buffer::Iterator i) const
{
...
uint32_t
NrMacPduHeader::Deserialize (Buffer::Iterator i)
{
...
void
NrMacPduHeader::Print (std::ostream &os) const
{
}

void
NrMacPduHeader::AddSubheader (const MacSubheader &macSubheader)
{
