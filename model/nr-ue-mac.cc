    ...
    static TypeId tid =
        TypeId("ns3::NrUeMac")
            ...
            .AddAttribute("Tproc0",
                          "t_proc0 in slots",
                          UintegerValue(1),
                          MakeUintegerAccessor(&NrUeMac::SetTproc0, &NrUeMac::GetTproc0),
                          MakeUintegerChecker<uint8_t>())
            .AddAttribute("T1",
                          "The start of the selection window in physical slots, accounting for "
                          "physical layer processing delay",
                          UintegerValue(2),
                          MakeUintegerAccessor(&NrUeMac::SetT1, &NrUeMac::GetT1),
                          MakeUintegerChecker<uint8_t>())
            .AddAttribute("T2",
                          "The end of the selection window in physical slots",
                          UintegerValue(33),
                          MakeUintegerAccessor(&NrUeMac::SetT2, &NrUeMac::GetT2),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute(
                "ActivePoolId",
                "The pool id of the active pool used for TX and RX",
                UintegerValue(0),
                MakeUintegerAccessor(&NrUeMac::SetSlActivePoolId, &NrUeMac::GetSlActivePoolId),
                MakeUintegerChecker<uint16_t>())
            .AddAttribute(
                "ReservationPeriod",
                ...
                MakeTimeAccessor(&NrUeMac::SetReservationPeriod, &NrUeMac::GetReservationPeriod),
                MakeTimeChecker())
            ...
            .AddAttribute(
                ...
                IntegerValue(-128),
                MakeIntegerAccessor(&NrUeMac::SetSlThresPsschRsrp, &NrUeMac::GetSlThresPsschRsrp),
...
NrUeMac::GetNrSlTxOpportunities(const SfnSf& sfn)
{
    ...
    if (m_enableSensing)
    {
        ...
        if (rvIt != sensedData.crend())
        {
            ...
            {
                NS_LOG_DEBUG("IMSI " << m_imsi << " ignoring sensed SCI at sfn " << sfn
...
NrUeMac::UpdateSensingWindow(const SfnSf& sfn)
{
    ...
    while (it != m_sensingData.cend())
    {
        ...
        {
            NS_LOG_DEBUG("IMSI " << m_imsi << " erasing SCI at sfn " << sfn << " received at "
                                 << it->sfn);
        ...
        if (m_nrSlHarq->GetNumAvaiableHarqIds() > 0)
        {
            for (const auto& itDst : m_sidelinkTxDestinations)
            {
                ...
                if (foundDest)
                {
                    ...
                    {
                        NS_LOG_INFO("IMSI " << m_imsi << " keeping the resource for "
                                            << itDst.first);
...
void
NrUeMac::DoReportNrSlBufferStatus(
...
void
NrUeMac::DoAddNrSlLc(const NrSlUeCmacSapProvider::SidelinkLogicalChannelInfo& slLcInfo,
                     NrSlMacSapUser* msu)
{
