...
SystemSchedulerTestQos::SystemSchedulerTestQos (uint32_t ueNumPergNb,
                                                uint32_t numerology, double bw1,
                                                bool isDownlnk, bool isUplink,
                                                double p1, double p2,
...
SystemSchedulerTestQos::DoRun ()
{
  ...
  if (verbose)
    {
      std::cout << "Test case: Scheduler type: " << m_schedulerType <<
                   " numerology: " << m_numerology << " BW: " << m_bw1 <<
                   " DL: " << m_isDownlink << " UL: " << m_isUplink <<
  ...
  if (m_isDownlink)
  ...
  else
    {
      ...
      remoteHostStaticRoutingLowLat = ipv4RoutingHelper.GetStaticRouting (remoteHostLowLat->GetObject<Ipv4> ());
      remoteHostStaticRoutingLowLat->AddNetworkRouteTo (Ipv4Address ("7.0.0.0"), Ipv4Mask ("255.0.0.0"), 1);
      remoteHostStaticRoutingVoice = ipv4RoutingHelper.GetStaticRouting (remoteHostVoice->GetObject<Ipv4> ());
      remoteHostStaticRoutingVoice->AddNetworkRouteTo (Ipv4Address ("8.0.0.0"), Ipv4Mask ("255.0.0.0"), 1);
    }
  ...
  if (m_isUplink)
    {
      ...
      for (uint32_t j = 0; j < ueLowLatContainer.GetN (); ++j)
        {
          ulClientLowlat.SetAttribute ("RemoteAddress", AddressValue (internetIpIfacesLowLat.GetAddress (1)));
      ...
      for (uint32_t j = 0; j < ueVoiceContainer.GetN (); ++j)
        {
          ulClientVoice.SetAttribute ("RemoteAddress", AddressValue (internetIpIfacesVoice.GetAddress (1)));
  ...
  if (m_isDownlink)
    {
      ...
      if (verbose)
      {
          ...
          std::cout << "ratio: " << qciRatio << " throughput ratio: " <<
                       throughputRatio << std::endl;
      }

      NS_TEST_ASSERT_MSG_EQ_TOL (qciRatio, throughputRatio, (qciRatio * 0.1),
