...
#include "test-mmwave-spectrum-phy.h"
...
/**
 * \ingroup nr-tests
 * \defgroup MmWaveSpectrumPhy test cases
 */
...
SetNoisePsdTestCase::SetNoisePsdTestCase (double txPower, double bandwidth, double noiseFigureFirst, double noiseFigureSecond,
  ...
  : TestCase ("MmWaveSpectrumPhy configuration test case")
{
  m_txPower = txPower;
...
SetNoisePsdTestCase::DoRun (void)
{
  ...
  Ptr<MmWaveSpectrumPhy> rxPhy = CreateObject<MmWaveSpectrumPhy> ();
  ...
  Ptr<const SpectrumModel> sm =  MmWaveSpectrumValueHelper::GetSpectrumModel (m_bandwidth, centerFrequency, m_numerology);
  Ptr<const SpectrumValue> txPsd = MmWaveSpectrumValueHelper::CreateTxPowerSpectralDensity  (m_txPower, sm);
  Ptr<const SpectrumValue> nsv0first = MmWaveSpectrumValueHelper::CreateNoisePowerSpectralDensity (m_noiseFigureFirst, sm);
  Ptr<const SpectrumValue> nsv0second = MmWaveSpectrumValueHelper::CreateNoisePowerSpectralDensity (m_noiseFigureSecond, sm);

  Ptr<MmwaveSpectrumSignalParametersDataFrame> params1 = Create<MmwaveSpectrumSignalParametersDataFrame>();
  params1->duration = Time (MilliSeconds(1));
  params1->psd = Copy(txPsd);
  params1->cellId = 99;
  Ptr<MmWaveSpectrumPhy> txPhy = CreateObject<MmWaveSpectrumPhy>();
  txPhy->SetMobility(CreateObject<ConstantPositionMobilityModel>());
  params1->txPhy = txPhy;

  rxPhy->GetMmWaveInterference()->TraceConnectWithoutContext("SnrPerProcessedChunk",
                                                              MakeBoundCallback (&TestSaveSnr, this));

  Simulator::Schedule (MilliSeconds(0), &MmWaveSpectrumPhy::SetNoisePowerSpectralDensity, rxPhy, nsv0first);
  Simulator::Schedule (MilliSeconds(1), &MultiModelSpectrumChannel::StartTx, spectrumChannel, params1);
  Simulator::Schedule (MilliSeconds(3), &mmWaveInterference::EndRx, rxPhy->GetMmWaveInterference());

  Simulator::Schedule (MilliSeconds(4), &MmWaveSpectrumPhy::SetNoisePowerSpectralDensity, rxPhy, nsv0second);
  Simulator::Schedule (MilliSeconds(5), &MultiModelSpectrumChannel::StartTx, spectrumChannel, params1);
  Simulator::Schedule (MilliSeconds(7), &mmWaveInterference::EndRx, rxPhy->GetMmWaveInterference());
...
MmWaveSpectrumPhyTestSuite::MmWaveSpectrumPhyTestSuite ()
  : TestSuite ("test-mmwave-spectrum-phy")
{
  ...
  for (auto input: testInputValuesSet)
    {
      uint64_t effectiveBandwidth = MmWaveSpectrumValueHelper::GetEffectiveBandwidth (input.bandwidth, input.numerology);
 ...
 * Test suite variable
 */
static MmWaveSpectrumPhyTestSuite g_mmwaveSpectrumPhyTestSuite;
