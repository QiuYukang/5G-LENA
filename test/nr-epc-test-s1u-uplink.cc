 ...
 * stamp but also including the NrQosFlowTag. This tag is normally
 ...
 *
 */
class NrQosFlowTagUdpClient : public Application
{
  public:
    /**
     ...
     * @param bid the BID
     */
    NrQosFlowTagUdpClient(uint16_t rnti, uint8_t bid);

    ~NrQosFlowTagUdpClient() override;
  ...
  private:
    ...
    uint8_t m_qfi;   ///< the QoS flow ID
};

TypeId
NrQosFlowTagUdpClient::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NrQosFlowTagUdpClient")
            .SetParent<Application>()
            .AddConstructor<NrQosFlowTagUdpClient>()
            .AddAttribute(
                "MaxPackets",
                "The maximum number of packets the application will send (zero means infinite)",
                UintegerValue(100),
                MakeUintegerAccessor(&NrQosFlowTagUdpClient::m_count),
                MakeUintegerChecker<uint32_t>())
            .AddAttribute("Interval",
                          "The time to wait between packets",
                          TimeValue(Seconds(1.0)),
                          MakeTimeAccessor(&NrQosFlowTagUdpClient::m_interval),
                          MakeTimeChecker())
            .AddAttribute("RemoteAddress",
                          "The destination Ipv4Address of the outbound packets",
                          Ipv4AddressValue(),
                          MakeIpv4AddressAccessor(&NrQosFlowTagUdpClient::m_peerAddress),
                          MakeIpv4AddressChecker())
            .AddAttribute("RemotePort",
                          "The destination port of the outbound packets",
                          UintegerValue(100),
                          MakeUintegerAccessor(&NrQosFlowTagUdpClient::m_peerPort),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("PacketSize",
                          "Size of packets generated. The minimum packet size is 12 bytes which is "
                          "the size of the header carrying the sequence number and the time stamp.",
                          UintegerValue(1024),
                          MakeUintegerAccessor(&NrQosFlowTagUdpClient::m_size),
                          MakeUintegerChecker<uint32_t>());
    return tid;
}

NrQosFlowTagUdpClient::NrQosFlowTagUdpClient()
    : m_rnti(0),
      m_qfi(0)
{
...
NrQosFlowTagUdpClient::NrQosFlowTagUdpClient(uint16_t rnti, uint8_t bid)
    : m_rnti(rnti),
      m_qfi(bid)
{
    NS_LOG_FUNCTION_NOARGS();
...
void
NrQosFlowTagUdpClient::SetRemote(Ipv4Address ip, uint16_t port)
{
    m_peerAddress = ip;
    m_peerPort = port;
}
...
void
NrQosFlowTagUdpClient::DoDispose()
{
    NS_LOG_FUNCTION_NOARGS();
...
void
NrQosFlowTagUdpClient::StartApplication()
{
    NS_LOG_FUNCTION_NOARGS();
    ...
    m_sendEvent = Simulator::Schedule(Seconds(0.0), &NrQosFlowTagUdpClient::Send, this);
}

void
NrQosFlowTagUdpClient::StopApplication()
{
    NS_LOG_FUNCTION_NOARGS();
...
NrQosFlowTagUdpClient::Send()
{
    ...
    {
        m_sendEvent = Simulator::Schedule(m_interval, &NrQosFlowTagUdpClient::Send, this);
    }
}
...
NrEpcS1uUlTestCase::DoRun()
{
    ...
    for (auto gnbit = m_gnbUlTestData.begin(); gnbit < m_gnbUlTestData.end(); ++gnbit)
    {
        ...
        for (uint32_t u = 0; u < ues.GetN(); ++u)
        {
            ...
            Ptr<NrQosFlowTagUdpClient> client =
                CreateObject<NrQosFlowTagUdpClient>(gnbit->ues[u].rnti, gnbit->ues[u].bid);
